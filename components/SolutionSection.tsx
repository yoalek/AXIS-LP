  ...
  const steps = [
    {
      ...
      desc: 'Através de workshops de descoberta e pesquisa, começamos definindo uma estratégia alinhada com sua visão de negócio.',
    ...
    {
      number: '02',
      title: 'Planejamento & Priorização',
      desc: 'Mapeamos a jornada do colaborador, marcos do projeto e entregáveis garantindo um workflow transparente do início ao fim.',
    ...
    {
      number: '03',
      title: 'Execução & Melhoria',
      desc: 'Implementamos as soluções com monitoramento contínuo, ajustando processos em tempo real para máxima eficiência operacional.',
              ...
              {steps.map((step, index) => (
                <div key={index} className="flex gap-8 md:gap-16 group relative mb-24 last:mb-0">
                  <div className="flex flex-col items-center flex-shrink-0 relative z-20">
                    <div className={`w-12 h-12 md:w-16 md:h-16 rounded-full border-2 transition-all duration-500 flex items-center justify-center text-[15px] font-bold bg-card shadow-sm group-hover:scale-110 ${
                      progressHeight > (index / steps.length) * 100 + 10 
                    ...
                    <InView transition={{ delay: index * 0.1, duration: 0.8 }}>
                      ...
                      <div className="bg-secondary/30 rounded-[2rem] p-8 border border-border group-hover:border-primary/30 group-hover:bg-secondary/50 transition-all duration-500">
                        ...
                        <div className="mt-8 flex gap-2">
                          {[1, 2, 3].map(i => (
                            <div key={i} className={`h-1 rounded-full transition-all duration-1000 delay-${i * 100} ${
                              progressHeight > (index / steps.length) * 100 + 20 
                              ? 'bg-primary w-12' 
                              : 'bg-muted w-4'
                            }`}></div>
                          ))}
